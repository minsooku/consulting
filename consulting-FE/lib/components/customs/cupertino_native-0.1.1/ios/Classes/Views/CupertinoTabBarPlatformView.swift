...
class CupertinoTabBarPlatformView: NSObject, FlutterPlatformView, UITabBarDelegate {
  ...
  init(
    ...
    let count = max(labels.count, max(symbols.count, assetIcons.count))
    ...
    self.currentIconSizes = sizes.map { CGFloat(truncating: $0) }
...
channel.setMethodCallHandler { [weak self] call, result in
      ...
      case "setLayout":
        if let args = call.arguments as? [String: Any] {
          ...
          let assetIcons = self.currentAssetIcons
          let sizes = self.currentIconSizes.map { NSNumber(value: Double($0)) }
          let appearance: UITabBarAppearance? = {
            if #available(iOS 13.0, *) { let ap = UITabBarAppearance(); ap.configureWithDefaultBackground(); return ap }
            return nil
          }()
  ...
  private func iconImage(
    ...
    let pointSize: CGFloat = {
      if index < sizes.count {
        let s = CGFloat(truncating: sizes[index])
        return s > 0 ? s : 24
      }
      return 24
    }()

    if index < assetIcons.count {
      let assetPath = assetIcons[index]
      if !assetPath.isEmpty {
        if let image = imageFromFlutterAssetPath(assetPath) {
          return resizedIconImage(image, pointSize: pointSize).withRenderingMode(.alwaysOriginal)
        }
      }
    }
  ...
  private func imageFromFlutterAssetPath(_ assetPath: String) -> UIImage? {
    let lookupKey = registrar.lookupKey(forAsset: assetPath)
    if let image = UIImage(named: lookupKey) {
      return image
    }
    let bundlePath = Bundle.main.bundlePath
    let filePath = "\(bundlePath)/\(lookupKey)"
    return UIImage(contentsOfFile: filePath)
  }

  private func resizedIconImage(_ image: UIImage, pointSize: CGFloat) -> UIImage {
    ...
    return renderer.image { _ in
      // Clip to a circle and use aspect-fill so the circle is fully filled.
