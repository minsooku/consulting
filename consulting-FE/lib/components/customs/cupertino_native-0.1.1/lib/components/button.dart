...
class CNButton extends StatefulWidget {
  ...
  /// Creates a button with both an SF Symbol icon and a text label.
  const CNButton.iconLabel({
    super.key,
    required this.label,
    required this.icon,
    this.onPressed,
  ...
  /// Button icon (null in text-only mode).
  final CNSymbol? icon;
...
class _CNButtonState extends State<CNButton> {
  ...
  Widget build(BuildContext context) {
    if (!(defaultTargetPlatform == TargetPlatform.iOS ||
      ...
      Widget child;
      if (widget.round) {
        child = Icon(CupertinoIcons.ellipsis, size: widget.icon?.size);
      } else if (widget.icon != null && widget.label != null) {
        child = Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(CupertinoIcons.square_grid_2x2, size: widget.icon?.size ?? 16),
      ...
      return SizedBox(
        ...
        width: widget.round ? (widget.width ?? widget.height) : null,
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (widget.round) {
          ...
          // While waiting for the native intrinsic-width response, estimate
          // from label length so longer labels (e.g. "Bottom Right") never
          // get clipped or wrapped inside a too-narrow SizedBox.
          final estimated = widget.label != null
              ? (widget.label!.length * 10.0 + 36.0).clamp(80.0, 400.0)
              : 80.0;
          width = _intrinsicWidth ?? estimated;
        }
        return Listener(
          onPointerDown: (e) {
  ...
  Future<void> _syncPropsToNativeIfNeeded() async {
    ...
    if (_lastTitle != widget.label && widget.label != null) {
      // Approximate width immediately so the native view never wraps the new
      // text while waiting for the async intrinsic-size response.
      final approx = (widget.label!.length * 8.5 + 32.0).clamp(80.0, 400.0);
      if (_intrinsicWidth == null || approx > _intrinsicWidth!) {
        setState(() => _intrinsicWidth = approx);
      }
