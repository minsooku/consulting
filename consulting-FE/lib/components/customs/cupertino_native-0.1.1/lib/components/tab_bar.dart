...
class CNTabBarItem {
  ...
  const CNTabBarItem({this.label, this.icon, this.assetIconPath});
  ...
  /// Optional Flutter asset image path (e.g. `assets/images/profile.png`).
  ///
  /// If provided for an item, the native tab bar will render this image instead
  /// of SF Symbol for that tab.
...
class _CNTabBarState extends State<CNTabBar> {
  ...
  Widget build(BuildContext context) {
    ...
    final assetIcons = widget.items.map((e) => e.assetIconPath ?? '').toList();
  ...
  void _cacheItems() {
    ...
    _lastAssetIcons = widget.items.map((e) => e.assetIconPath ?? '').toList();
  }
